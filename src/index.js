...
app.use(express.static(__dirname + "/view"), (_, res, next) => {
    res.status(404);
...
console.log(`\u001b[0m`);
console.log(chalk.red(`© Piper | 2021 - ${new Date().getFullYear()}`));
...
manager.on("shardCreate", (shard) => {
    ...
    shard.on("shardReconnecting", () => {
        const embed = new Discord.EmbedBuilder().setTitle(`🚨・Reconnecting shard ${shard.id + 1}/${manager.totalShards}`).setColor(config.colors.normal);
        shardLogs.send({
            username: "Bot Logs",
            embeds: [embed],
        });
